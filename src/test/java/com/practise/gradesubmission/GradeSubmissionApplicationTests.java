...
public class GradeSubmissionApplicationTests{
	...
	void setup(){
		...
		gradeRepository.saveAll(Arrays.asList(
			new Grade("B+", studentRepository.findById(1L).get(), courseRepository.findById(1L).get()),
			new Grade("A+", studentRepository.findById(3L).get(), courseRepository.findById(5L).get())
		));
	}
